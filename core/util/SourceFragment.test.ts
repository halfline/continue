...
describe('Completion behavior', () => {
  ...
  it('correctly identifies completion between partial prefix and suffix lines', () => {
    ...
    // Test that we can get just the part before the suffix
  ...
  it('handles whitespace-only last line with mergeWhitespace option', () => {
    ...
    // Create a partial fragment that ends with just whitespace
    const partialWithWhitespace = new SourceFragment("function test() {\n  ");
    
    // With mergeWhitespace:true, it should match even with the whitespace-only last line
    ...
    // Test with a partial that ends with a newline after whitespace
